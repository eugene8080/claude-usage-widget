...
Outputs (relative to watchface-terminal/):
    ...
    resources-mesh/drawables/drawables.xml + mesh_tile.png + scan_tile.png
    source-glow/GlowTimeMetrics.mc

Run from anywhere:  python watchface-terminal/tools/build_glow_time.py   (all THEMES; edit that table)
"""
...
# darkened by SCAN_DIM - horizontal only, like an old monitor. Same 114 px tile so it lines up
# with the screen the same way the mesh does.
...
def main() -> int:
    ...
    scan = np.zeros((MESH_TILE, MESH_TILE, 4), np.uint8)
    scan[..., 3] = np.where((yy % SCAN_PITCH) == SCAN_PITCH - 1, int(round(SCAN_DIM * 255)), 0)
    Image.fromarray(scan, "RGBA").save(MESH_DIR / "scan_tile.png", optimize=True)
    (MESH_DIR / "drawables.xml").write_text(
        "<drawables>\n    <!-- Generated by tools/build_glow_time.py - full-face VFD mesh tile, and the\n"
        "         horizontal-only CRT scanline tile used instead when the Scanlines setting is on. -->\n"
